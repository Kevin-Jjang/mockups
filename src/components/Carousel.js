  ...
  return (
    <div className={`${carouselStyles.base} ${carouselStyles.base__genre}`}>
      <div className={carouselStyles.viewer}>
        {
          items.map((item, idx) => {
            return <Carousel__GenreItem key={idx} item={item}
              style={{ transform: `translate(-${curItem * 100}%)` }} />
          })
        }
      </div>
      <div className={`${carouselStyles.control} ${carouselStyles['control--right']}`}>
...
function Carousel__GenreItem({ item, style }) {
  return (
    <div className={`${homepageStyles.block} ${carouselStyles.item}`} style={style}>
      <div className={homepageStyles.block__img}>
        <div className={homepageStyles.block__img__container}>
          <Image className={`${homepageStyles.img__cover} ${homepageStyles["img__cover--left"]}`}
      ...
      <div className={homepageStyles.block__text}>
        <p className={'text--heading_2'}>{item.title}</p>
